...
@Controller
public class ProductInsertController2  implements ServletContextAware {
	...
	@Autowired // ServletContext객체를 여기서 @Autowired를 이용해서 주입해도 되고, 저 아래 setServletContext()에서
				// 주입해도 된다.
	...
	public ModelAndView doActionPost(@ModelAttribute("product") @Valid Product product,
			BindingResult bindingResult, ServletConfig sc , MultipartHttpServletRequest mtfRequest) { // 매개변수로
																											// ServletContext
																											// servletContext를
																											// 쓰니 에러난다.
		// jsp에서 name = upload에 다중화일 들어간다. 
		...
		//
		 // ServletContext servletContext = sc.getServletContext();
		  //System.out.println("servletContext:"+servletContext); String scontext =
		  //servletContext.getContextPath(); System.out.println("scontext:"+scontext);
		...
		String uploadPath4 = servletContext.getRealPath("/resources");
		...
		/
		  아래 추가 ProductValidator validator = new ProductValidator();
		  validator.setDao(productDao); validator.validate(product,bindingResult);
		  /////여기까지 추가 /////
		 /

		System.out.println("product.getName():" + product.getName()); // prod-servlet.xml에 multipartResolver 설정 안해놓으면 이
																		// 부분에서 null이 나올 수 있다.
		...
		if (bindingResult.hasErrors()) {
		  ...
		  MultipartFile multi = product.getUpload(); // multi에는 name이 upload인 곳의 정보가
		  들어간다.
		  
		  // 하드디스크 c드라이브의 temp 폴더에 이미지 업로드
		  
		  String pathname = "c:\\temp\\"; //File target = new File(pathname); File
		  target = new File(uploadPath4);
		  
		  if (!target.exists()) {// 폴더가 없으면 target.mkdir();// 폴더 강제로 생성 }
		  ...
		  // 데이터베이스에 추가하는 코드 Integer cnt = -1; cnt = productDao.InsertData(product);
		  
		  if (cnt != -1) { // 성공 mav.setViewName(gotoPage); // redirect:/list.prd
		  
		  System.out.println("1:"+multi.getOriginalFilename()+"/"+multi.getName()); //
		  1:child2.jpg/upload //File destination = new File(pathname +
		  multi.getOriginalFilename()); File destination = new File(uploadPath4 +
		  File.separator + multi.getOriginalFilename()); // 윗줄에서
		  multi.getOriginalFilename()대신 product.getImage()도 된다.
		  
		  
		  실제 화일이 올라가는 위치
		  C:\Spring_STS_my\.metadata\.plugins\org.eclipse.wst.server.core\tmp3\
		  wtpwebapps\51_Spring_MyBatis_Products\resources
		  
		  
		  try { multi.transferTo(destination); // transferTo:원하는 위치에 저장할 때 사용 //File
		  객체를 만든 뒤 MultipartFile 객체의 transferTo 메서드를 실행시켜 실제 물리적인 파일을 만든다. // 업로드 한 파일
		  데이터를 지정한 파일에 저장한다. // 이 줄이 없으면 그림이 보이지 않는다.
		  
		  } catch (IllegalStateException e) { e.printStackTrace(); } catch (IOException
		  e) { e.printStackTrace(); }
		  
		  } else {// 실패 mav.setViewName(getPage); //ProductInsertForm.java
        ...
        for (MultipartFile mf : fileList) {
            ...
            String safeFile = uploadPath4 + File.separator + System.currentTimeMillis() + originFileName;
            try {
...
//	 Auto-generated method stub servletContext=context; }
//	 
}
*/
